...
public class Main : BaseUnityPlugin
{
	...
	private void Update()
	{
		...
		foreach (var pair in Nametags.Where(p =>
			         !NConfig.ShowingNametags || !GorillaParent.instance.vrrigs.Contains(p.Key)))
		{
			pair.Value.Destroy();
		...
		foreach (var rig in GorillaParent.instance.vrrigs.Where(rig =>
			         NConfig.ShowingNametags && rig != GorillaTagger.Instance.offlineVRRig))
		{
	...
	private static string GetPlatformString(VRRig player)
	{
		...
		return cosmetics.Contains("S. FIRST LOGIN") ? "steam" : properties > 1 ? "oculus" : "meta";
	}
